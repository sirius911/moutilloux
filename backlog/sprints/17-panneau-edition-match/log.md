# Log — Sprint 17 : Panneau d'édition de match

| # | Date | Verdict spec | Dérives | Nouvelles issues | Issues sprint restantes |
|---|------|-------------|---------|-----------------|------------------------|
| — | 2026-07-02 | — | Sprint planifié (audit admin) | #189-194 (+ #7, #168, #175 réutilisées) | 9 |
| #1 | 2026-07-02 | ❌ | 11 dérives (9 confirmées toujours présentes + 2 nouvelles majeures) | #220, #221 (+ #222 hors périmètre sprint, sans milestone) | 11 |
| #2 | 2026-07-02 | ⚠️ | 11 dérives (9 confirmées toujours présentes + 2 nouvelles) | #224 (majeure), #225 (mineure) | 11 |
| #3 | 2026-07-02 | ⚠️ | 8 dérives (toutes déjà connues, 0 nouvelle) — #175 vérifiée résolue et fermée | 0 | 8 |
| #4 | 2026-07-02 | ⚠️ | 7 dérives (6 confirmées toujours présentes + 1 nouvelle) | #226 (mineure) | 7 |
| #5 | 2026-07-02 | ⚠️ (cycle-de-vie-match.md → ✅) | 6 dérives (5 confirmées toujours présentes + 1 nouvelle) | #227 (mineure, duplication reopen) | 6 |
| #6 | 2026-07-02 | ⚠️ (cycle-de-vie-match.md toujours ✅) | 4 dérives (toutes déjà connues, 0 nouvelle) | 0 | 4 |
| #7 | 2026-07-02 | ⚠️ (cycle-de-vie-match.md toujours ✅) | 2 dérives (toutes déjà connues, 0 nouvelle) — #193/#194 revérifiées sans régression | 0 | 2 |
| #8 | 2026-07-03 | ❌ (admin-matchs.md ⚠️, cycle-de-vie-match.md ❌ — régression sur #227) | 2 dérives (2 nouvelles) | #228 (majeure/bloquante), #229 (mineure) | 2 |
